...
export type Branding = { logo: string | null; favicon: string | null };
...
function fetchBranding(): Promise<Branding> {
  if (!inflight) {
    inflight = api<any>("/platform/public/site-config/")
      .then((d) => ({ logo: d?.logo ?? null, favicon: d?.favicon ?? null }))
      .catch(() => ({ logo: null, favicon: null }));
  }
  return inflight;
}
...
export function useBranding(): Branding {
  const [b, setB] = useState<Branding>(cache || { logo: null, favicon: null });
  useEffect(() => {
    let alive = true;
